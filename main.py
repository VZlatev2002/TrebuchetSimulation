...
pi = np.pi
L_BC = 0 #distance between the pivot and counterweight
L_S = 0 # the sling length 
H = 0 # height
L_BP = L_B - L_BC #8.5
...
#L_BC = 1.5 distance between the pivot and counterweight
#L_S = 6 # the sling length 
#H = 6 # height
...
length_x = []
x_axis = []
L_BCa = np.arange(1, 1.4, 0.5)
L_Sa = np.arange(5, 5.4, 0.5)
Ha = np.arange(5, 5.4,0.5)
...
def main(i,j,k):
    ...
    # ani = tf.animate_trebuchet(t1, z1, t2, z2, t3, z3, p)      
t1 = time.time()
x_y = Parallel(n_jobs=1)(delayed(main)(i,j,k) for i in L_BCa for j in L_Sa for k in Ha)
print(x_y)
